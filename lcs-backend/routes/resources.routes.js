...
router.post('/upload', authenticate, requireRole('Administrator', 'Teacher'), upload.single('file'), asyncHandler(async (req, res) => {
  ...
  const { fileUrl, fileData } = await uploadResourceFile(req.file.buffer, req.file.originalname);
