...
// STAFF_DELETE_ROLES below), and unlike the old version of this route,
// no longer blocked by existing payroll history. allowances.staff_id,
// salary_advances.staff_id, and payroll_records.staff_id are all
// `REFERENCES staff_profiles(id) ON DELETE CASCADE` (see schema.sql), so
// the database itself removes every allowance, salary advance, and
// payroll record tied to this staff member the moment their profile is
// deleted — there's nothing left to manually clean up first. HR/Director
// (who can still edit/view staff via PAYROLL_EDIT_ROLES) no longer have
// delete access at all; they still have PUT /staff/:id to set someone
// "inactive" instead, which is the safer, non-destructive option for
// day-to-day use.
