...
// Activity, one row per lessonPhases entry, in Introduction -> Lesson
// Development -> Evaluation -> Conclusion order) + a Teacher's Self
// Assessment footer — backend content shape
// (config/aiTools/lessonPlan.js): { theme, topic, competency,
// learningOutcomes: string[], genericSkills: string[], values: string[],
// crossCuttingIssues: string[], keyLearningOutcome, preRequisiteKnowledge,
// references: string[], lessonPhases: [{ phase, durationMinutes,
// teacherActivity, learnerActivity }], teacherSelfAssessment: string[] }.
