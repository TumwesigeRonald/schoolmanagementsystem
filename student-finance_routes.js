...
// Same fallback convention as resolveTermYear() in scores.routes.js: an
// explicit term/year in the query always wins (so a student can look back
// at a past term), otherwise fall back to term_settings' current row.
...
// GET /api/student-finance/my-balance?term=&year=
// Student-only. Returns this student's billed/paid/balance for the given
// (or current) term, plus their own payment history for that term — the
// same shape finance.routes.js's GET /payments?studentId= returns for
// staff, just always forced to the caller's own ID.
router.get('/my-balance', authenticate, requireRole('Student'), asyncHandler(async (req, res) => {
  ...
  // Same billed/paid calc as finance.routes.js's GET /payments — kept in
  // sync deliberately: billed = this student's override for the term if
  // one is set, otherwise their class's default fee, otherwise 0.
  ...
  res.json({
    ...
    // Deliberately omits `reference`, `note`, and `recordedBy` here —
    // those are internal Bursar-office details with no reason to be
    // shown back to the student; method + amount + date is enough for
    // them to recognise a payment as theirs.
    history
  });
}));
